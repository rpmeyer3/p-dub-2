...
const DEFAULT_MARKER_COLOR: [number, number, number] = [0.3, 0.45, 0.85];
const DEFAULT_BASE_COLOR: [number, number, number] = [1, 1, 1];
const DEFAULT_ARC_COLOR: [number, number, number] = [0.3, 0.45, 0.85];
const DEFAULT_GLOW_COLOR: [number, number, number] = [0.94, 0.93, 0.91];
...
export function Globe({
  ...
  useEffect(() => {
    ...
    function init() {
      ...
      // Cast: cobe's standard types don't include arcs/markerElevation/etc.
      // Extra props are passed through; library ignores unknowns.
    ...
    <div className={`relative aspect-square select-none ${className}`}>
      <canvas
        ...
        style={{
          ...
          touchAction: "none",
          background:
            "radial-gradient(circle, #000 0%, #000 56%, transparent 50%)",
        }}
      />
      {markers.map((m) => (
        <div
